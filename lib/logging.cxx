...
Logging::Dict::iterator find_sink(const LogTag &tag, Logging::Dict &dict) {
  Logging::Dict::iterator it =
      std::find_if(dict.begin(), dict.end(), [&](Logging::Dict::value_type &v) {
...
Logging::mktmpdir(const boost::filesystem::path &model) {
...
bool Logging::clear_all() {
  ...
  std::for_each(
      Logging::loggers_.begin(), Logging::loggers_.end(),
      [&](std::unordered_map<LogTag, boost::weak_ptr<TextSink>>::value_type
              &pair) {
